...
static debug_test_context_t g_debug_ctx;  // Debug test context
#endif
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "Firmware Version: 1.0 (DEBUG BUILD)");

    // Initialize system
    system_init();

#ifdef CONFIG_DEBUG_ENABLE
    // Initialize debug test system
    ESP_LOGI(TAG, "=== DEBUG MODE ENABLED ===");
    ESP_LOGI(TAG, "Test Phase: %d", CONFIG_DEBUG_TEST_PHASE);
    ESP_LOGI(TAG, "Auto-run Tests: %s", CONFIG_DEBUG_AUTO_RUN_TESTS ? "YES" : "NO");
    ESP_LOGI(TAG, "Verbose Logging: %s", CONFIG_DEBUG_VERBOSE_LOGGING ? "YES" : "NO");

    debug_test_init(&g_debug_ctx, CONFIG_DEBUG_TEST_PHASE, CONFIG_DEBUG_AUTO_RUN_TESTS, MY_NODE_ID);

    // Link subsystems to debug context
    g_debug_ctx.node = &g_node;
    g_debug_ctx.audio = &g_audio;
    g_debug_ctx.network = &g_network;
    g_debug_ctx.poke_queue = g_poke_queue;
#endif
...
#ifdef CONFIG_DEBUG_ENABLE_MIDI_SIMULATOR
    ESP_LOGI(TAG, "Starting MIDI simulator task");
    xTaskCreatePinnedToCore(
        debug_midi_simulator_task,
        "midi_sim",
        4096,
        &g_debug_ctx,
        2,
...
#ifdef CONFIG_DEBUG_ENABLE_HUB_SIMULATOR
    ESP_LOGI(TAG, "Starting Hub simulator task");
    xTaskCreatePinnedToCore(
        debug_hub_simulator_task,
        "hub_sim",
        4096,
        &g_debug_ctx,
        2,
...
#ifdef CONFIG_DEBUG_MONITOR_TASK_ENABLED
    ESP_LOGI(TAG, "Starting debug monitor task");
    xTaskCreatePinnedToCore(
        debug_monitor_task,
        "debug_mon",
        4096,
    ...
    // Run tests if auto-run enabled
    if (CONFIG_DEBUG_AUTO_RUN_TESTS) {
        ESP_LOGI(TAG, "Auto-running test phase %d", CONFIG_DEBUG_TEST_PHASE);
        vTaskDelay(pdMS_TO_TICKS(2000));  // Wait for tasks to stabilize

        if (debug_test_run_phase(&g_debug_ctx)) {
            ESP_LOGI(TAG, "Test phase %d completed successfully", CONFIG_DEBUG_TEST_PHASE);
        } else {
            ESP_LOGE(TAG, "Test phase %d failed", CONFIG_DEBUG_TEST_PHASE);
        }

        debug_test_print_summary(&g_debug_ctx);
    }
#endif
